...
// NÃO entra na cascata: é sempre digitada manualmente pelo usuário. Todos os
// campos preenchidos pela cascata continuam editáveis manualmente depois.
...
// (precisa refletir exatamente ANDAMENTO_STAGES do Code.gs).
...
document.getElementById('sofGrid').addEventListener('click', function (e) {
  const card = e.target.closest('[data-sof-id]');
  if (card) openSofEditModal(card.getAttribute('data-sof-id'));
});
...
function renderSOFCard(sof) {
  ...
  const andamento = sof.ANDAMENTO || 'Não iniciado';
  const andamentoClass = sof.ANDAMENTO === 'NE EMITIDA'
    ? 'bg-green-100 text-green-700'
    : (sof.ANDAMENTO ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-600');

  return (
    '<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex flex-col gap-2 cursor-pointer hover:shadow-md hover:border-blue-300 transition" ' +
         'data-sof-id="' + escapeHtml(sof.ID) + '">' +
      '<div class="flex items-start justify-between gap-2">' +
        '<h3 class="font-semibold text-slate-800">' + escapeHtml(sof.UNIDADE || 'Unidade não informada') + '</h3>' +
        '<span class="text-xs px-2 py-0.5 rounded-full whitespace-nowrap ' + andamentoClass + '">' + escapeHtml(andamento) + '</span>' +
      '</div>' +
...
// ===================== MODAL SOF (criar e editar) =====================

function populateAndamentoOptions() {
  const select = document.getElementById('andamentoInput');
  select.innerHTML = ANDAMENTO_STAGES.map(function (etapa) {
    return '<option value="' + escapeHtml(etapa) + '">' + escapeHtml(etapa) + '</option>';
  }).join('');
}

/** Abre o modal em modo criação (sofData omitido) ou edição (sofData preenchido). */
function openSofModal(sofData) {
  const form = document.getElementById('sofForm');
  form.reset();

  if (sofData) {
    state.editingSofId = sofData.ID;
    document.getElementById('sofModalTitle').textContent = 'Editar SOF - ' + (sofData.NUM_SOF || sofData.UNIDADE || '');
    document.getElementById('sofSubmitBtn').textContent = 'Salvar Alterações';

    Object.keys(FORM_FIELD_TO_COLUMN).forEach(function (fieldId) {
      const el = document.getElementById(fieldId);
      const coluna = FORM_FIELD_TO_COLUMN[fieldId];
      if (!el || !sofData.hasOwnProperty(coluna)) return;
      if (el.type === 'checkbox') {
        el.checked = !!sofData[coluna];
      } else {
        el.value = sofData[coluna] === null || sofData[coluna] === undefined ? '' : sofData[coluna];
      }
    });
  } else {
    state.editingSofId = null;
    document.getElementById('sofModalTitle').textContent = 'Nova Solicitação de Orçamento (SOF)';
    document.getElementById('sofSubmitBtn').textContent = 'Salvar SOF';
...
function openSofEditModal(id) {
  const sof = state.sofs.find(function (s) { return String(s.ID) === String(id); });
  if (sof) openSofModal(sof);
}
...
document.getElementById('novaSofBtn').addEventListener('click', function () { openSofModal(); });
...
document.getElementById('sofForm').addEventListener('submit', async function (e) {
  ...
  try {
    if (state.editingSofId) {
      await apiRequest('updateSOF', { id: state.editingSofId, data: data });
      showToast('SOF atualizada com sucesso!', 'success');
    } else {
