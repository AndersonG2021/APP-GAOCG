...
function criarUsuario(session, dados) {
  ...
  var frente = perfil === 'gerente' ? '' : sanitizeString_(dados.frente, 50);
  ...
  if (perfil === 'analista' && FRENTES.indexOf(frente) === -1) {
    return fail_('Selecione uma frente válida para o analista.');
  }
...
function atualizarUsuario(session, id, dados) {
  ...
  if (dados.hasOwnProperty('frente')) atualizado.frente = atualizado.perfil === 'gerente' ? '' : sanitizeString_(dados.frente, 50);
  if (atualizado.perfil === 'analista' && FRENTES.indexOf(atualizado.frente) === -1) {
    return fail_('Selecione uma frente válida para o analista.');
  }
