...
`id, unidade_id, oss_snapshot, cnpj_snapshot, divergente_da_unidade, tipo_unidade, objeto, contrato_gestao, parcela_contratual, fonte, nota_empenho, competencia, valor_liquidado, valor_pago, nota_liquidacao_drive_id, nota_liquidacao_url, ordem_bancaria, ordem_bancaria_arquivo_drive_id, ordem_bancaria_arquivo_url, numero_processo, observacao, status, parcela_dividida_grupo_id, percentual_parcela_dividida, alerta_divergencia_valores, completo, origem, criado_por, data_criacao, data_ultima_alteracao_status, visualizado_apos_alerta, excluido, excluido_por, excluido_em` (`contrato_gestao` renomeado de `instrumento` em 2026-08-31 - mesmo campo da unidade vinculada)
...
- Filtros de listagem: unidade_id, oss (exato), status (exato), competencia (exato), fonte (exato), tipo_unidade (exato), dea (via join Recibo→NotaEmpenho→SOF.dea por igualdade de `nota_empenho`), substring em objeto/contrato_gestao/nota_empenho/numero_processo, e busca livre global. Sempre exclui `excluido=true`.
...
### 6.4 Recibos
- Autopreenchimento: escolher a Unidade já preenche `contrato_gestao` (renomeado de `instrumento` em 2026-08-31) com o valor cadastrado nela; escolher Objeto depois busca o Recibo histórico mais recente da mesma combinação e sobrescreve com `contrato_gestao, parcela_contratual, fonte, nota_empenho` daquele registro, se existir (e refiltra o dropdown de Status pela nova fonte). No backend, `contrato_gestao` sempre tem a Unidade como fallback quando chega vazio (montarLinhaRecibo_) - não depende mais de herdar de um Recibo anterior pra nunca ficar em branco (ver gerarRecibosMeta).
...
- Tela é uma **tabela** (não cards, diferente de SOF/Unidades/NE), com linhas destacadas em amarelo quando `destacar_parado` e badge vermelho "!" quando `alerta_divergencia_valores`. Extensa barra de filtros: busca, unidade, OSS, objeto, tipo de unidade, DEA, competência, fonte, status, contrato de gestão, nota de empenho, nº processo.
